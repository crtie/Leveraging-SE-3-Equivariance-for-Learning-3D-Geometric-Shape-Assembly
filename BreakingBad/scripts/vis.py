...
def visualize(cfg):
    ...
    for batch in tqdm(val_loader):
        ...
        batch["part_rot"] = Rotation3D(part_quat, rot_type="quat").convert(
            model.rot_type
        )
        out_dict = model.forward(batch)  # trans/rot: [B, P, 3/4/(3, 3)]
        # loss_dict, _ = model._calc_loss(out_dict, batch)  # loss is [B]
        loss_dict, _ = model._loss_function(batch, out_dict)  # loss is [B]
        # the criterion to cherry-pick examples
        if model.rot_type == "quat":
            loss = loss_dict["rot_pt_l2_loss"] + loss_dict["trans_mae"]
        else:
            loss = loss_dict["rot_loss"] + loss_dict["trans_loss"]
            out_dict["rot"] = Rotation3D(out_dict["rot"], rot_type="rmat")
    ...
    for rank, idx in enumerate(top_idx):
        ...
        cur_save_dir = os.path.join(
            save_dir, mesh_dir.split("/")[-2], subfolder_name
        )
        ...
        for i, mesh_file in enumerate(mesh_files):
            ...
            init_pc = trimesh.sample.sample_surface(
                init_mesh, val_dst.num_points
            )[0]
            save_pc(
                init_pc,
            ...
            pred_trans, pred_quat = (
                out_dict["pred_trans"][i],
                out_dict["pred_quat"][i],
            )
            ...
            pred_pc = trimesh.sample.sample_surface(
                pred_mesh, val_dst.num_points
            )[0]
            save_pc(
                pred_pc,
