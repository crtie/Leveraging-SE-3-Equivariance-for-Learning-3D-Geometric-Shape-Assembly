  ...
  for(int cut_idx=0; cut_idx<5; cut_idx++)
  {
    ...
    while(instance_idx < num_instances)
    {
      ...
      {      
        ...
        double D_mesh_volume = 1.0;
        int L_num_of_components = 0, R_num_of_components = 0;
        double L_mesh_volume = 0.0, R_mesh_volume = 0.0;
        double volume_ratio = 0.25;
        int counter = 0;
        while(L_num_of_components != 1 || R_num_of_components != 1 || L_mesh_volume < volume_ratio * D_mesh_volume || R_mesh_volume < volume_ratio * D_mesh_volume)
        {
          ...
          std::cout << "Shrinking the mesh...\n";
          double iso_level = -0.05;
          int s = 20; // Control the reslution of marching cube
          igl::SignedDistanceType sd_type = igl::SIGNED_DISTANCE_TYPE_FAST_WINDING_NUMBER;
          Eigen::MatrixXd shrinked_mesh_v, GV, S;
          Eigen::MatrixXi shrinked_mesh_f, side;
          offset_surface(tmp_mesh_v, mesh_f, iso_level, s, sd_type, shrinked_mesh_v, shrinked_mesh_f, GV, side, S);

          std::cout << "Computing the difference between the outer mesh and the inner mesh...\n";
          igl::MeshBooleanType mesh_minus = igl::MESH_BOOLEAN_TYPE_MINUS;
          Eigen::MatrixXd D_mesh_v;
          Eigen::MatrixXi D_mesh_f, D_birth_place;
          igl::copyleft::cgal::mesh_boolean(tmp_mesh_v, mesh_f, shrinked_mesh_v, shrinked_mesh_f, mesh_minus, D_mesh_v, D_mesh_f, D_birth_place);

          // Compute the volume of the difference mesh
          get_mesh_volume(D_mesh_v, D_mesh_f, D_mesh_volume);

          std::cout << "Trimming the mesh...\n";
          igl::MeshBooleanType mesh_intersect = igl::MESH_BOOLEAN_TYPE_INTERSECT;
          igl::copyleft::cgal::mesh_boolean(D_mesh_v, D_mesh_f, cut_surface_v, cut_surface_f, mesh_intersect, L_mesh_v, L_mesh_f, L_birth_place);
          igl::copyleft::cgal::mesh_boolean(D_mesh_v, D_mesh_f, cut_surface_v, cut_surface_f, mesh_minus, R_mesh_v, R_mesh_f, R_birth_place);
          ...
          std::cout << "L mesh volume: " << L_mesh_volume << "\n";
          std::cout << "R mesh volume: " << R_mesh_volume << "\n";
          std::cout << "D mesh volume: " << D_mesh_volume << "\n";
          counter += 1;
          if(counter == 5)
          {
            break;
        ...
        // shell root dir
        std::string shell_root_dir = object_root_dir + "shell/";
        if(!std::filesystem::is_directory(shell_root_dir))
          std::filesystem::create_directory(shell_root_dir);

        // cut root dir
        std::string cut_root_dir = shell_root_dir + cut_type + "/";
